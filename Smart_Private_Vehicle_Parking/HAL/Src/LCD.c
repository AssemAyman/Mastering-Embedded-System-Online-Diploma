...
GPIOx_REG* LCD_PORT[2] = {LCD1_PORT,LCD2_PORT};
...
void LCD_Kick(int index){
	MCAL_GPIOx_WritePin(LCD_PORT[index],EN[index],GPIO_PIN_SET);
	wait_ms(10);
	MCAL_GPIOx_WritePin(LCD_PORT[index],EN[index],GPIO_PIN_RESET);
}

void LCD_Send_Command (unsigned char CMD,int index){
	//turn RS off for command mode.
	MCAL_GPIOx_WritePin(LCD_PORT[index],RS[index],GPIO_PIN_RESET);
	...
	MCAL_GPIOx_WritePin(LCD_PORT[index], D4, Read_Bit(CMD,4));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D5, Read_Bit(CMD,5));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D6, Read_Bit(CMD,6));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D7, Read_Bit(CMD,7));
	LCD_Kick(index);
	//Write the command on D0..D3
	MCAL_GPIOx_WritePin(LCD_PORT[index], D4, Read_Bit(CMD,0));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D5, Read_Bit(CMD,1));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D6, Read_Bit(CMD,2));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D7, Read_Bit(CMD,3));
	LCD_Kick(index);
#endif
}
...
void LCD_Init(){
	...
	for(int j=0; j<2; j++){
		...
		//RS OUTPUT Push-Pull
		Pincfg.GPIO_PinNumber = RS[j];
		Pincfg.GPIO_MODE = GPIO_MODE_OUTPUT_PP;
		Pincfg.GPIO_OUTPUT_SPEED = GPIO_SPEED_10M;
		MCAL_GPIOx_Init(LCD_PORT[j],&Pincfg);

		//EN OUTPUT Push-Pull
		Pincfg.GPIO_PinNumber = EN[j];
		Pincfg.GPIO_MODE = GPIO_MODE_OUTPUT_PP;
		Pincfg.GPIO_OUTPUT_SPEED = GPIO_SPEED_10M;
		MCAL_GPIOx_Init(LCD_PORT[j],&Pincfg);

		//turn off enable
		MCAL_GPIOx_WritePin(LCD_PORT[j],EN[j],GPIO_PIN_RESET);
		...
		for(i=0; i<8; i++){
			...
			Pincfg.GPIO_MODE = GPIO_MODE_OUTPUT_PP;
			Pincfg.GPIO_OUTPUT_SPEED = GPIO_SPEED_10M;
			MCAL_GPIOx_Init(LCD_PORT[i],&Pincfg);
		}
...
void LCD_Send_A_Character (char data,int index){
	...
	MCAL_GPIOx_WritePin(LCD_PORT[index],RS[index],GPIO_PIN_SET);
...
#ifdef FOUR_Bit_MODE
	//send D4..D7 first
	MCAL_GPIOx_WritePin(LCD_PORT[index], D4, Read_Bit(data,4));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D5, Read_Bit(data,5));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D6, Read_Bit(data,6));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D7, Read_Bit(data,7));
	LCD_Kick(index);
	//send data D0..D3
	MCAL_GPIOx_WritePin(LCD_PORT[index], D4, Read_Bit(data,0));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D5, Read_Bit(data,1));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D6, Read_Bit(data,2));
	MCAL_GPIOx_WritePin(LCD_PORT[index], D7, Read_Bit(data,3));
	LCD_Kick(index);
#endif
}
